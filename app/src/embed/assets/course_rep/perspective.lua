...
extern float SIZEY;//So annoying
...
{
	return vec4(over.rgb*over.a+base.rgb*(1.0),over.a+base.a*(1.0));
}
vec4 effect(vec4 colour,Image UNUSED1,vec2 UNUSED2,vec2 inverted)
{
	...
	if(eq0.x && eq0.y){
		...
		//uv
		number r=length(p-pA)/pow(1.0-ct*ct,.5);
	...
	return colour*Texel(img,mod(uv*rep+vec2(p0.x-1.0,p0.y),one));
}
]]
